...
public class RowDataFileScanTaskReader implements FileScanTaskReader<RowData> {
  ...
  @Override
  public CloseableIterator<RowData> openDelete(
      FileScanTask task, InputFilesDecryptor inputFilesDecryptor) {
    Schema partitionSchema = TypeUtil.select(projectedSchema, task.spec().identitySourceIds());
  ...
  private CloseableIterable<RowData> newEqDeleteParquetIterable(
      ...
      Map<Integer, ?> idToConstant,
      InputFilesDecryptor inputFilesDecryptor) {
    if (task.deletes().stream()
        .noneMatch(file -> file.content().equals(FileContent.EQUALITY_DELETES))) {
    ...
    Iterable<CloseableIterable<RowData>> iterable =
        Iterables.transform(
            inputFiles,
            inputFile -> {
              Parquet.ReadBuilder builder =
                  Parquet.read(inputFile)
                      ...
                      .project(schema)
                      .createReaderFunc(
                          fileSchema -> FlinkParquetReaders.buildReader(schema, fileSchema))
